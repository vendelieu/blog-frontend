...
export class TagsService {
  ...
  getByName(slug: string): Observable<Tag[] | undefined> {
    return this.apiService
      .httpGet<Tag[]>(this.apiService.getApiUrl(ApiUrl.GET_TAG_BY_NAME).replace(':name', slug))
      .pipe(map((res) => res.data || undefined));
  }

  create(tag: NewTag): Observable<string | undefined> {
    return this.apiService
      .httpPost<string>(this.apiService.getApiUrl(ApiUrl.ADMIN_TAG_ACTION), tag)
      .pipe(map((res) => res.data || undefined));
  }

  link(postSlug: string, tagSlug: string): Observable<string | undefined> {
    return this.apiService
      .httpPut<string>(this.apiService.getApiUrl(ApiUrl.ADMIN_TAG_ACTION), {
        post_slug: postSlug,
        tag_slug: tagSlug
      })
      .pipe(map((res) => res.data || undefined));
