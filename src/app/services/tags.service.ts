...
export class TagsService {
  ...
  create(tag: NewTag): Observable<string | undefined> {
    return this.apiService.httpPost<string>(
      this.apiService.getApiUrl(ApiUrl.ADMIN_TAG_ACTION),
      tag
    ).pipe(map((res) => res.data || undefined));
  }
  ...
  link(postSlug: string, tagSlug: string): Observable<string | undefined> {
    return this.apiService.httpPut<string>(
      this.apiService.getApiUrl(ApiUrl.ADMIN_TAG_ACTION),
      {post_slug: postSlug, tag_slug: tagSlug}
    ).pipe(map((res) => res.data || undefined));
  }

  unlink(postSlug: string, tagSlug: string): Observable<string | undefined> {
    return this.apiService.httpDelete<string>(
      this.apiService.getApiUrl(ApiUrl.ADMIN_TAG_UNLINK)
        .replace(':slug', tagSlug).replace(':post_slug', postSlug)
    ).pipe(map((res) => res.data || undefined));
  }
}
